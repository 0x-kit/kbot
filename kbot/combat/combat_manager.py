# combat/combat_manager.py - FIXED VERSION
import time
...
class CombatManager:
    """Enhanced combat manager with full skills integration - FIXED VERSION"""
    ...
    def __init__(self, pixel_analyzer: PixelAnalyzer, skill_manager: SkillManager, 
        ...
        # 🔧 CRITICAL FIX: Track valid vs invalid targets separately
        self.last_valid_target_time = 0  # NEW: Track when we last had a VALID target
        self.invalid_target_streak = 0   # NEW: Count consecutive invalid targets
    ...
    def _evaluate_target(self, target_exists: bool, target_name: str) -> bool:
        """FIXED: Evaluate if we have a valid target to attack"""
        current_time = time.time()
        
        # 🔧 CRITICAL FIX: Only reset stuck timer for VALID targets
        if target_exists and self._is_target_allowed(target_name):
            self.stuck_start_time = 0  # Only reset for valid targets
            self.last_valid_target_time = current_time
            self.invalid_target_streak = 0
        elif target_exists and not self._is_target_allowed(target_name):
            # Invalid target detected - don't reset stuck timer
            self.invalid_target_streak += 1
            self.logger.debug(f"Invalid target streak: {self.invalid_target_streak}")
        
        self.logger.debug(f"Target evaluation: exists={target_exists}, name='{target_name}', current='{self.current_target}', valid_target_allowed={self._is_target_allowed(target_name) if target_name else False}")
    ...
    def _handle_no_target(self, current_time: float) -> None:
        """FIXED: Handle situation when we have no valid target with better movement logic"""
        
        # 🔧 CRITICAL FIX: Start stuck timer when we first lose valid targets
        if self.stuck_start_time == 0:
            self.stuck_start_time = current_time
            self.logger.debug("Started stuck timer - no valid target found")
        
        time_stuck = current_time - self.stuck_start_time
        
        # 🔧 ENHANCED: More aggressive movement when stuck
        if time_stuck > self.timing['stuck_detection_time']:
            self.logger.warning(f"No valid target for {time_stuck:.1f}s - executing anti-stuck movement")
            self._execute_smart_movement()
            self.stuck_start_time = current_time  # Reset timer after movement
        ...
        # 🔧 IMPROVED: Move more frequently when only invalid targets are present
        time_since_last_movement = current_time - self.last_movement
        
        # Conditions for movement:
        # 1. Been stuck for more than 3 seconds
        # 2. Haven't moved recently
        # 3. Either no targets at all OR only invalid targets for a while
        should_move = (
            time_stuck > 3.0 and  # Been without valid targets for 3+ seconds
            time_since_last_movement > self.timing['movement_interval'] and
            (self.invalid_target_streak > 5 or time_stuck > 5.0)  # Too many invalid targets or stuck too long
        )
        
        if should_move:
            self.logger.info(f"No valid targets for {time_stuck:.1f}s (invalid streak: {self.invalid_target_streak}) - moving to search")
    ...
    def process_combat(self) -> None:
        ...
        try:
            ...
            if (self.state == CombatState.FIGHTING and 
                ...
                self._handle_combat_with_skills(current_time)
                return
    ...
    def _try_use_skill(self, current_time: float, target_health: int) -> bool:
        """Try to use the next appropriate skill with correct step logging"""
        try:
    ...
    def _spiral_movement(self) -> None:
        """Spiral movement pattern for better area coverage"""
        try:
    ...
    def _exploration_movement(self) -> None:
        """Exploration movement - longer distance in new directions"""
        try:
            import random
            ...
            turn_direction = random.choice(['a', 'd'])
            turn_duration = random.uniform(1.5, 3.0)
            walk_duration = random.uniform(4.0, 6.0)
    ...
    def _move_to_find_targets(self) -> None:
        """Move around to find new targets with better logic"""
        try:
            self.state = CombatState.MOVING
    ...
    def _basic_search_movement(self) -> None:
        ...
        try:
            ...
            if pattern == "turn_and_walk":
                ...
                self.input_controller.hold_key('d', turn_duration)
                time.sleep(0.2)
                self.input_controller.hold_key('w', walk_duration)
                
            elif pattern == "circle_pattern":
                for i in range(3):
                    self.input_controller.hold_key('d', 0.4)
                    time.sleep(0.1)
                    self.input_controller.hold_key('w', 1.5)
                    time.sleep(0.2)
    ...
    def configure_movement_behavior(self, aggressive_search: bool = True) -> None:
        ...
        if aggressive_search:
            self.timing['movement_interval'] = 3.0
            self.timing['stuck_detection_time'] = 10.0
            self.logger.info("Configured for aggressive movement (faster searching)")
        else:
            self.timing['movement_interval'] = 8.0
            self.timing['stuck_detection_time'] = 20.0
    ...
    def get_movement_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_valid_target_time': self.last_valid_target_time,
            'invalid_target_streak': self.invalid_target_streak,
            'time_since_valid_target': current_time - self.last_valid_target_time
        }