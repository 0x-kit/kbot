...
class InputController:
    ...
    using win32api.PostMessage for reliable background operation.
    """
    ...
    def __init__(
        self, window_manager: WindowManager, logger: Optional[BotLogger] = None
    ):
    ...
    def _make_lparam(self, vk_code: int, event_type: str) -> int:
        """Constructs a realistic lParam for keyboard events."""
    ...
    def send_key(self, key: str) -> bool:
        """Sends a key press (down and up) to the target window."""
        hwnd = self._get_target_hwnd()
        if not hwnd:
            self.logger.warning(
                "Attempted to send key, but no target window is selected."
            )
        ...
        if key_lower not in self.VK_CODE:
            self.logger.error(f"Key '{key}' is not in VK_CODE map.")
            self.input_stats["failed_inputs"] += 1
            return False
        ...
        try:
            lParam_down = self._make_lparam(vk_code, "down")
            lParam_up = self._make_lparam(vk_code, "up")

            win32api.PostMessage(hwnd, win32con.WM_KEYDOWN, vk_code, lParam_down)
            time.sleep(0.05 + random.uniform(0.01, 0.03))  # Realistic delay
    ...
    def click_at(self, x: int, y: int, button: str = "left") -> bool:
        """Sends a click to specific screen coordinates, converting them to window client coordinates."""
        hwnd = self._get_target_hwnd()
        if not hwnd:
            return False

        try:
            # Convierte las coordenadas de pantalla a coordenadas relativas al cliente de la ventana.
            # Esto es CRUCIAL para que el clic caiga en el lugar correcto dentro del juego.
            ...
            time.sleep(0.05 + random.uniform(0.01, 0.02))
            ...
            self.input_stats["failed_inputs"] += 1
            self.logger.error(f"Failed to send click at screen ({x},{y}): {e}")
            return False

    # --- Métodos de estadísticas y hold_key (que no usaremos para movimiento) ---
    def hold_key(self, key: str, duration: float) -> bool:
        """Holds a key. Note: Not reliable for game movement."""
        # Esta función se mantiene por si se necesita para otra cosa, pero no para movimiento.
        hwnd = self._get_target_hwnd()
        if not hwnd:
            return False
        key_lower = key.lower()
